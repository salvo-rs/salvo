...
use opentelemetry_semantic_conventions::{resource, trace, attribute};
use salvo_core::http::headers::{self, HeaderMapExt, HeaderMap, HeaderName, HeaderValue};
    ...
    async fn handle(&self, req: &mut Request, depot: &mut Depot, res: &mut Response, ctrl: &mut FlowCtrl) {
        ...
        async move {
            ...
            if let Some(content_length) = res.headers().typed_get::<headers::ContentLength>() {
                span.set_attribute(KeyValue::new(attribute::HTTP_RESPONSE_BODY_SIZE, content_length.0 as i64));
            }
        }
