...
use salvo_core::http::{header, StatusError};
use salvo_core::routing::FlowCtrl;
use salvo_core::{async_trait, Depot, Handler, Request, Response, Router};
use serde::Serialize;
...
impl Handler for SwaggerUi {
    async fn handle(&self, req: &mut Request, _depot: &mut Depot, res: &mut Response, _ctrl: &mut FlowCtrl) {
        let mut file_path = req.params().get("**file_path").map(|s| &**s).unwrap_or_default();
        ...
        // } else if let Some(doc) = self.docs.get(file_path) {
        //     res.write_body(doc.clone()).ok();
        // } else {
...
/// ```rust
/// # use salvo_swagger_ui::{Config, Url};
...
/// With oauth config.
/// ```rust
/// # use salvo_swagger_ui::{Config, oauth};
...
/// }
/// ```
pub fn serve<'a>(path: &str, config: Arc<Config<'a>>) -> Result<Option<SwaggerFile<'a>>, Box<dyn Error>> {
...
/// Is used to provide general way to deliver multiple types of OpenAPI docs.
#[cfg(any(feature = "actix-web", feature = "rocket", feature = "axum"))]
#[derive(Clone)]
enum ApiDoc {
    Utoipa(salvo::openapi::OpenApi),
...
impl Serialize for ApiDoc {
        ...
        S: serde::Serializer,
    {
        match self {
            ...
            Self::Utoipa(salvo) => salvo.serialize(serializer),
        }
    }
}
