...
/// Represents the parameters passed by Cookie.
pub struct OptionalCookieParam<T>(pub Option<T>);
impl<T> OptionalCookieParam<T> {
    /// Consumes self and returns the value of the parameter.
...
impl<T> Deref for OptionalCookieParam<T> {
...
impl<T> DerefMut for OptionalCookieParam<T> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.0
    }
}

impl<'de, T> Deserialize<'de> for OptionalCookieParam<T>
where
    T: Deserialize<'de>,
{
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        T::deserialize(deserializer).map(|value| OptionalCookieParam(Some(value)))
    }
}

impl<T> fmt::Debug for OptionalCookieParam<T>
where
    T: fmt::Debug,
{
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
...
#[async_trait]
impl<'de, T> Extractible<'de> for OptionalCookieParam<T>
where
    T: Deserialize<'de>,
{
    fn metadata() -> &'de Metadata {
...
impl<T> EndpointArgRegister for OptionalCookieParam<T>
where
    T: ToSchema,
{
    fn register(components: &mut Components, operation: &mut Operation, arg: &str) {
        let parameter = Parameter::new(arg)
            .parameter_in(ParameterIn::Cookie)
            .description(format!("Get optional parameter `{arg}` from request cookie."))
            .schema(T::to_schema(components))
            .required(false);
        operation.parameters.insert(parameter);
    }
