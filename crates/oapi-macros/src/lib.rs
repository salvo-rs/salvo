...
///
/// * `paths(...)`  List of method references having attribute [`#[salvo_oapi::path]`][path] macro.
...
///   See more details in [`#[salvo_oapi::path(...)]`][path] [attribute macro security options][path_security].
...
///   Alternatively the tag name can be given to path operation via [`#[salvo_oapi::path(...)]`][path] macro.
...
/// ```
/// # use salvo_oapi::{OpenApi, ToSchema};
/// #
/// #[derive(ToSchema)]
/// struct Pet {
///     name: String,
...
/// }
///
/// #[salvo_oapi::path(get, path = "/status")]
...
pub fn openapi(input: TokenStream) -> TokenStream {
    ...
    let openapi_attributes = openapi::parse_openapi_attrs(&attrs)
        .expect_or_abort("expected #[openapi(...)] attribute to be present when used with OpenApi derive trait");
