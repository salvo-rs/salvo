...
use salvo_core::conn::{
    Accepted, Acceptor, HandshakeStream, Holding, HyperHandler, HttpBuilder, Listener,
    StraightStream, CancellationToken,
};
use salvo_core::fuse::{ArcFuseFactory, FuseInfo, TransProto};
...
impl Acceptor for TlsDetectAcceptor {
    ...
    fn accept(
        ...
        let holdings_local_addr = self.holdings.first().map(|h| h.local_addr.clone());
        ...
        async move {
            ...
            let local_addr = holdings_local_addr
                .unwrap_or_else(|| tcp_stream.local_addr().unwrap_or(remote_addr).into());

            // Create fusewire for connection monitoring.
            ...
            if buf[0] == TLS_HANDSHAKE_BYTE {
                ...
                let plain_stream = StraightStream::new(tcp_stream, fusewire.clone());
                let tls_stream = HandshakeStream::new(tls_acceptor.accept(plain_stream), fusewire.clone());
