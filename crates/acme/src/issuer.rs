...
pub(crate) async fn issue_cert(
    ...
    if !valid {
        return Err(CoreError::other("authorization failed too many times"));
    }
    // send csr
    let mut params = CertificateParams::new(config.domains.clone())
        .map_err(|e| CoreError::other(format!("crate certificate params failed: {e}")))?;
    params.distinguished_name = DistinguishedName::new();

    let key_pair = KeyPair::generate()
        .map_err(|e| CoreError::other(format!("generate key pair failed: {e}")))?;

    let csr = params
        .serialize_request(&key_pair)
        .map_err(|e| CoreError::other(format!("failed to serialize request der {e}")))?;
    ...
    let cert_pem = client
        .obtain_certificate(
            order_res
                .certificate
                .as_ref()
                .ok_or_else(|| CoreError::other("invalid response: missing `certificate` url"))?,
        )
        .await?
        .as_ref()
        .to_vec();
...
fn gen_acme_cert(domain: &str, acme_hash: &[u8]) -> CoreResult<CertifiedKey> {
    let key_pair = KeyPair::generate()
        .map_err(|e| CoreError::other(format!("generate key pair failed: {e}")))?;

    let mut params = CertificateParams::new(vec![domain.to_owned()])
        .map_err(|e| CoreError::other(format!("create certificate params failed: {e}")))?;
    params.custom_extensions = vec![CustomExtension::new_acme_identifier(acme_hash)];
    let cert = params
        .self_signed(&key_pair)
        .map_err(|_| CoreError::other("failed to generate acme certificate"))?;
    let pk = any_ecdsa_type(&PrivateKeyDer::Pkcs8(PrivatePkcs8KeyDer::from(
        key_pair.serialize_der(),
    )))
    .expect("serialize private key der failed");
