...
impl HttpBuilder {
        ...
        I: AsyncRead + AsyncWrite + Unpin + Send + 'static,
    {
        ...
        let (version, socket) = crate::proto::read_version(socket).await?;
...
pub(crate) async fn read_version<'a, A>(mut reader: A) -> IoResult<(HttpVersion, Rewind<A>)>
where
    A: AsyncRead + Unpin,
{
    let mut buf = [0; 24];
...
{
    type Output = IoResult<(HttpVersion, Vec<u8>)>;

    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<IoResult<(HttpVersion, Vec<u8>)>> {
        let this = self.project();
