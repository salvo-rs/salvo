...
fn create_posts(
    ...
    // ✅ Insert into DB
    let row_affcted = diesel::insert_into(posts::table)
        .values(&new_post)
        .execute(&mut conn)
...
fn update_posts(
    ...
    let row_affcted = diesel::update(posts::table.find(post_uuid))
        .set((
            posts::title.eq(&update_data.title),
            posts::content.eq(&update_data.content),
