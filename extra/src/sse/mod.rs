...
//! use salvo::{Filter, sse::ServerSentEvent};
//! use futures::{stream::iter, Stream};
//!
//! fn sse_events() -> impl Stream<Item = Result<impl ServerSentEvent, Infallible>> {
//!     iter(vec![
//!         Ok(salvo::sse::data("unnamed event").into_a()),
//!         Ok((
//!             salvo::sse::event("chat"),
//!             salvo::sse::data("chat message"),
//!         ).into_a().into_b()),
//!         Ok((
//!             salvo::sse::id(13),
//!             salvo::sse::event("chat"),
//!             salvo::sse::data("other chat message\nwith next line"),
//!             salvo::sse::retry(Duration::from_millis(5000)),
//!         ).into_b().into_b()),
//!     ])
//! }
//!
//! let app = salvo::path("push-notifications")
//!     .and(salvo::get())
//!     .handle(salvo::extra::sse::keep_alive().stream(sse_events())));
//! ```
//!
...
///
/// ```
/// let app = salvo::sse::last_event_id::<u32>();
///
...
///             Ok(salvo::sse::data("payload").boxed()),
///             // Named event with ID and retry timeout
///             Ok((
///                 salvo::sse::data("other message\nwith next line"),
///                 salvo::sse::event("chat"),
///                 salvo::sse::id(1),
///                 salvo::sse::retry(Duration::from_millis(15000))
///             ).boxed()),
...
///
/// async {
///     let app = salvo::path("sse").and(salvo::get()).map(|| {
///        salvo::sse::reply(event_stream())
///     });
///
///     let res = salvo::test::request()
///         .method("GET")
...
/// use salvo::{Filter, Stream, sse::ServerSentEvent};
///
/// // create server-sent event
/// fn sse_counter(counter: u64) ->  Result<impl ServerSentEvent, Infallible> {
///     Ok(salvo::sse::data(counter))
/// }
///
/// fn main() {
///     let routes = salvo::path("ticks")
///         .and(salvo::get())
///         .map(|| {
...
///             let stream = salvo::sse::keep_alive()
///                 .interval(Duration::from_secs(5))
///                 .text("thump".to_string())
///                 .stream(event_stream);
///             salvo::sse::reply(stream)
///         });
/// }
/// ```
///
